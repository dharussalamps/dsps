-- Import CSV dedupe rules (admin request, 2026-09-10):
--
-- 1. Admission number ("index number") is checked against existing students
--    *before* any insert is attempted. A match means that row is skipped
--    outright — no row is ever attempted twice, and the report carries a
--    clean 'duplicate_admission_no' reason instead of a raw unique-
--    violation sqlerrm bubbled up from the exception handler.
--
-- 2. Guardian dedupe now keys off nic_number (the same identity key
--    AddGuardianSection already dedupes by for the manual add flow — see
--    useGuardianByNic / 20260910080000_guardian_profile_fields.sql). If the
--    CSV's guardian_nic_number matches an existing guardian, that guardian
--    is linked as-is and every other guardian_* column in the row
--    (guardian_name, guardian_relationship, guardian_phone) is ignored —
--    the existing record is never overwritten by an import. Only when no
--    guardian_nic_number is given, or none matches, is a new guardian
--    created from the row's guardian_name/guardian_phone (as before).
