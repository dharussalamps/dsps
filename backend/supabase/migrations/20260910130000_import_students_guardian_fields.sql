-- Extends the CSV import's new-guardian creation (20260910120000) with the
-- same optional profile fields AddGuardianSection already collects for a
-- manually-added guardian (phone_alt, email, occupation, economic_status,
-- address, gs_division — see 20260910080000_guardian_profile_fields.sql and
-- 20260910090000_move_address_to_guardian.sql for where these columns came
-- from). Only used when a new guardian is created from the row; a
-- guardian_nic_number match still ignores every guardian_* column and links
-- the existing record as-is.
