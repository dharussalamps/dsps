...
attendance.amend_locked    attendance.early_leave      attendance.mark_staff
attendance.reopen_staff
...
| `vice_principal` | all except `leave.approve`, `audit.view`, `attendance.reopen_staff` | `school` |
...
| 3 | Auth: sign in, first-password set, reset by OTP, session handling | done | SignIn (email + password), SetPassword (first-login, see the `needs_password_set` convention below), and an OTP password-reset flow all built and wired through `authStore`/`RootNavigator`. **Reset channel switched from phone/SMS to email after initial build** (see the "Password reset method" deviation note below for why) — `signInWithOtp({ email })` + `verifyOtp({ email, token, type: 'email' })`. Session persists via AsyncStorage. **Not verified against a live project**: the dev seed's staff rows have no linked `auth.users` yet (see the seed file's own comment for how to link one). |
...
| 21 | User accounts, role assignment with scope, audit log viewer | done | Widened `write_staff`'s RLS policy to accept `account.manage` as well as `staff.manage` — creating the staff row is part of what "create a user account" means. `UserAccountsScreen` (create staff record, assign role+scope, deactivate/reactivate, and now "Create login" per staff member) and `AuditLogScreen` (most recent 100 entries). **Updated after initial build**: issuing a login (an `auth.users` row) needs Supabase's admin API, callable only with the service role — never from the app's own anon-key client — so a 9th Edge Function, `create-staff-login`, was added beyond section 8's listed 8 (justified the same way section 8 itself justifies one: "requires privileged work"). It re-checks `account.manage`/`staff.manage` server-side via the caller's own JWT before ever touching the service-role client, then creates an email-based `auth.users` row (requires an email already on the staff record; see the "Password reset method" deviation note for why email rather than phone) with a randomly generated temporary password and `user_metadata.needs_password_set = true`, and links `staff.auth_user_id`. The temporary password is emailed directly to the staff member via Gmail's SMTP server (the `denomailer` Deno library, using a `GMAIL_USER`/`GMAIL_APP_PASSWORD` secret pair) and is *also* returned once in the API response and surfaced to the principal in an alert, which stays the fallback whenever the email doesn't go out (missing secrets or any send failure — `email_sent` in the response tells the UI which happened). Gmail SMTP was chosen over a transactional email provider (Resend was tried first) because every such provider requires verifying a domain you control before it'll deliver to arbitrary recipients, and this school has no domain at all — Gmail's own domain is already trusted, so a personal Gmail account works with no domain of its own needed, and it's free (≈500 sends/day limit, far beyond this school's volume). Calling the same endpoint again for a staff member who already has a login doesn't 409 — it issues a *new* temporary password and resets their existing login to it (`created: false` in the response), since the original password is never stored anywhere to literally "resend." Not the "Forgot password" OTP flow: the staff member signs in normally (email + that password) via the ordinary `SignInForm`, and because `needs_password_set` is true, `RootNavigator` routes them straight to `SetPasswordScreen` before they can do anything else, per the existing convention in `src/store/authStore.ts`. "Forgot password" (OTP-by-email) remains for genuine later resets, not first-login. Supabase Studio / the admin API remains an option for out-of-band cleanup, but is no longer the only path. |
...
| 23 | Hardening: rate limiting, backup restore test, accessibility pass | in progress | **Rate limiting**: `check_rate_limit()` (pgTAP-tested) applied to `remind_unmarked_class` (1 per class per 2 min) and `create_announcement` (10/hour) — the two SECURITY DEFINER functions a user could realistically spam; the rest are naturally self-limiting (you can't usefully call `submit_attendance` twice for the same real event). Auth-level rate limits are Supabase dashboard configuration, documented in `backend/README.md` rather than faked in a migration. **Accessibility**: audited every `StatusPill` tone's text/background pair against WCAG AA (4.5:1) with a small script — `success`/`warning`/`info` measured 3.66/3.39/4.36 (all failing) and were darkened to 5.77/5.26/5.88; every table confirmed to have RLS enabled by diffing `create table` against `enable row level security` across every migration (43/43 match); `SyncStatusBadge`'s retry pill given `hitSlop` to clear the 44dp tap-target minimum despite its smaller visual size. **Backup/restore**: not testable in this environment (no live project) — documented as a checklist item in `backend/README.md` for whoever hosts this for real. |
...
- **First-login detection**: not specified by this document. An account
  created by an administrator (build task 21, `create-staff-login`) carries
  `user_metadata.needs_password_set = true` regardless of how its initial
  credential was set (originally passwordless + OTP; now a random
  temporary password the principal relays directly — see build task 21's
  note); `SetPassword` clears the flag after the user chooses their own
  password. `src/store/authStore.ts` is the reference.
- **Password reset method**: section 10 says `SignIn` offers "forgot
  ...
  channel. Originally implemented over phone/SMS rather than email,
  because `staff.phone` is required and `staff.email` is optional (section
  4.3) — an email-only reset would lock out any staff member without one
  on file. **Reversed after real SMS pricing was checked**: every
  Supabase-supported SMS provider (Twilio, Twilio Verify, MessageBird,
  Textlocal, Vonage) is pay-per-message with no ongoing free tier, and
  Twilio's rate to Sri Lankan numbers (+94) specifically runs to roughly
  $0.40/SMS — prohibitive for a school with no SMS budget. Supabase's own
  transactional email is free, so both `create-staff-login` (build task
  21) and the "forgot password" reset flow now use email exclusively. The
  trade-off named above is real and now accepted rather than avoided: a
  staff member with no email on file cannot get a login or reset a
  password until one is added to their record (via the account card's
  Edit action) — `staff.email` stays nullable at the schema level, so this
  is enforced operationally (a clear error naming the missing field), not
  by a `NOT NULL` migration that would break existing rows.
...
- **`create-staff-login` is a 9th Edge Function beyond section 8's list of
  8.** Unlike every other admin-screen write (RLS-checked PostgREST calls
  straight from the client), issuing a login means writing to
  `auth.users`, which only the service-role key can do — a key this app
  never holds and a client can never safely be trusted with. That's
  exactly the "requires privileged work" condition section 8 itself names
  as the bar for adding an Edge Function, so this one clears it on the
  same grounds `export-report` does. It re-verifies `account.manage`/
  `staff.manage` itself (via the caller's own JWT against `has_permission`)
  before switching to the service-role client, so the permission boundary
  is identical to the plain-PostgREST path — the extra hop exists only
  because Postgres itself has no privilege to create an `auth.users` row.
...
- FR-AUTH-01/03: sign-in is still by email (not `staff_no`), and the OTP
  reset flow still doesn't check the email belongs to a registered staff
